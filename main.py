...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="StockAgent — LLM-powered Stock Market Simulation")
    parser.add_argument(
        "--model",
        type=str,
        default="gemini-pro",
        help="LLM to use as agent brain. Options: gemini-pro, gpt-4, gpt-3.5-turbo",
    )
    args = parser.parse_args()

    # ── Startup Banner ──────────────────────────────────────────
    print("\n" + "=" * 60)
    print("   🚀  StockAgent — LLM Stock Market Simulation")
    print("=" * 60)
    print(f"   Model        : {args.model}")
    print(f"   Agents       : {util.AGENTS_NUM}")
    print(f"   Trading Days : {util.TOTAL_DATE}")
    print(f"   Sessions/Day : {util.TOTAL_SESSION}")
    print(f"   Stock A Init : ¥{util.STOCK_A_INITIAL_PRICE}")
    print(f"   Stock B Init : ¥{util.STOCK_B_INITIAL_PRICE}")
    print("=" * 60 + "\n")

    # ── API Key Validation ───────────────────────────────────────
    if "gpt" in args.model and not util.OPENAI_API_KEY:
        print("❌  ERROR: OPENAI_API_KEY is not set.")
        print("    Set it via:  $env:OPENAI_API_KEY='your-key'  (PowerShell)")
        print("                 export OPENAI_API_KEY='your-key' (Linux/Mac)")
        exit(1)
    if "gemini" in args.model and not util.GOOGLE_API_KEY:
        print("❌  ERROR: GOOGLE_API_KEY is not set.")
        print("    Set it via:  $env:GOOGLE_API_KEY='your-key'  (PowerShell)")
        print("                 export GOOGLE_API_KEY='your-key' (Linux/Mac)")
        exit(1)
